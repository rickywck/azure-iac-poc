...
class FoundryAgent:
    ...
    async def _execute_python_script(self, python_code: str) -> tuple[str, str]:
        # Local execution of LLM-generated code is prohibited: it allows arbitrary
        # code injection via a crafted user message or a manipulated LLM response.
        # All code execution must go through the sandboxed dynamic session executor.
        if not self.dynamic_session_executor_url:
            raise RuntimeError(
                "Code execution requires DYNAMIC_SESSION_EXECUTOR_URL to be configured. "
                "Local execution of LLM-generated code is disabled for security reasons."
            )
